...
def create_gerente_view(user_display_name: str, page: ft.Page, handle_logout):
    ...
    def show_bottom_status(
        ...
        try:
            if not bottom_bar_text_ref.current or not bottom_bar_ref.current:
                # refs ainda não anexados à página — não armazenar mensagens de boas-vindas
                return

            # Atualizar valores sem depender da atualização direta do controle,
            # pois o controle pode existir mas ainda não ter sido adicionado
            # de fato à árvore do page, o que causa "Text Control must be added"
            try:
            ...
            try:
                ...
                pass
            # Atualizar a página de forma segura
            try:
                page.update()
            except Exception:
                try:
    ...
    # Mensagens pendentes de boas-vindas foram removidas; nada a fazer aqui.
