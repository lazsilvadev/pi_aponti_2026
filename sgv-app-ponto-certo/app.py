    ...
    def handle_login(e):
        ...
        if caixa_user and selected_username == user and selected_autologin and not pwd:
            try:
                ...
                # Não exibir mensagem de boas-vindas rápida para contas específicas
                try:
                    skip_welcome = (caixa_user.username or "") in ("admin", "estoque1")
                except Exception:
                    skip_welcome = False
        ...
        if authenticated_user:
            ...
            try:
                ...
                pass
            # Preparar informações de usuário e decidir se pulamos mensagens
            try:
            ...
            skip_welcome = uname in ("admin", "estoque1")

            # Exibir snackbar de boas-vindas apenas quando não estiver na lista de skip
            if not skip_welcome:
                try:
                    show_snackbar(f"Bem-vindo, {authenticated_user.full_name}!", COLORS["green"])
                except Exception:
                    pass

            # Garantir que administradores/gerentes recebam a barra de boas-vindas
            try:
                if not skip_welcome and (uname == "admin" or roleval in ("gerente", "admin")):
                    try:
                        display_name = (
                            authenticated_user.full_name or authenticated_user.username
                        )
            ...
            try:
                ...
                if getattr(authenticated_user, "role", "") != "gerente" and not skip_welcome:
                    display_name = (
                        authenticated_user.full_name or authenticated_user.username
                    )
                    schedule_show_welcome_appbar(display_name, delay=3)
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                pass
                            # Somente mostrar para gerentes se não estiver na lista de skip
                            if not skip_welcome:
                                show_admin_bottom_bar(display_name, duration=6)
                        except Exception as ex:
                            try:
            ...
            try:
                ...
                # Exibir mensagem clara de senha inválida (mostrar na barra
                # inferior da tela de login quando disponível).
                try:
                    login_show = page.app_data.get("login_show_bottom_bar")
                    if callable(login_show):
                        login_show("Senha inválida", color=ft.Colors.RED_600)
                    else:
                        show_snackbar("Senha inválida", COLORS["red"])
                except Exception:
                    show_snackbar("Senha inválida", COLORS["red"])
            except Exception:
