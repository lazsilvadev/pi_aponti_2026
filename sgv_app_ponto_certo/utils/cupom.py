...
def show_cupom_dialog(
    ...
    try:
        if repassar_flag:
            # Se houver pagamentos parciais, somar acréscimo apenas sobre quantias pagas em Crédito
            if partial_payments:
                for p in partial_payments:
                    try:
                        if (p.get("method") or "") == "Crédito":
                            a = float(p.get("amount", 0) or 0)
                            acrescimo_total += a * (credito_pct / 100.0)
                    except Exception:
                        pass
            else:
                # Sem pagamentos parciais: aplicar sobre o total quando forma for Crédito
                try:
                    if (p_type or "") == "Crédito":
                        acrescimo_total = float(current_total or 0.0) * (
                            credito_pct / 100.0
                        )
                except Exception:
                    acrescimo_total = 0.0
    ...
    try:
        if acrescimo_total and float(acrescimo_total or 0.0) > 0.0:
            header_items.append(
                ft.Text(
