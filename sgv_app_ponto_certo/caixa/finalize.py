...
def finalize_transaction(
    ...
    try:
        ...
        try:
            ...
            if not sessao_caixa:
                try:
                    pdv_core.open_new_caixa(usuario_id or 0, 0.0)
                    show_snackbar(
                        "Caixa aberto automaticamente (saldo inicial R$ 0,00).",
                        COLORS["secondary"],
                    )
                    sessao_caixa = pdv_core.get_current_open_session(
                        usuario_id
                    ...
                    show_snackbar(
                        "O caixa está fechado e não pôde ser aberto automaticamente.",
                        COLORS["danger"],
                    )
                    set_finalizing(False)
                    return
