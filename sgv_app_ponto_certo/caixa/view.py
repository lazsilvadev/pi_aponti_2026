    ...
    def calculate_total():
        ...
        try:
            ...
            if selected_method:
                ...
                # Exemplo: só "Crédito" com repasse e taxa > 0 mostra subtotal/acréscimo
                if selected_method == "Crédito" and taxas.get("repassar", False):
                    pct = float(taxas.get("credito_avista", 0.0))
                    if pct > 0:
                        # considerar pagamentos já efetuados: acrescimo deve incidir apenas
                        # sobre o valor que será cobrado no Crédito (remaining)
                        try:
            ...
            if payment_amount_text_refs:
                for i, t_ref in enumerate(payment_amount_text_refs):
                    try:
                        ...
                        if method_paid > 0:
                            continue
                        # mostrar restante, aplicando acrescimo para Crédito quando necessário
                        if (
                            pm_name == "Crédito"
                            and repassar_tmp
                            and credito_pct_tmp
                            ...
                            total_with_acrescimo = remaining_now * (
                    ...
                    if method_paid > 0:
                        try:
                            ...
                            if (
                                ...
                                charged = method_paid * (1 + credito_pct_tmp / 100.0)
    ...
    def update_payment_status():
        try:
            ...
            try:
                ...
                # Se a forma selecionada for Crédito e houver repasse configurado,
                # exibir no campo 'Pago' o valor TOTAL cobrado ao cliente (com acréscimo),
                # em vez do subtotal. Caso contrário, mostrar o valor efetivamente pago.
                show_paid_as_total = False
                try:
                    sel = None
                    try:
                    ...
                    if sel == "Crédito":
                        ...
                        if repassar_tmp and credito_pct_tmp and credito_pct_tmp > 0:
                            # calcular o total com acréscimo para exibir em 'Pago'
                            total_com_acrescimo = total * (1 + credito_pct_tmp / 100.0)
                            show_paid_as_total = True
                except Exception:
                    show_paid_as_total = False

                if getattr(p_ref, "current", None):
                    try:
                        if show_paid_as_total:
                            p_ref.current.value = (
