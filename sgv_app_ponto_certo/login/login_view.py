...
def create_login_view(page, username_entry, password_entry, handle_login, COLORS):
    ...
    ordered_users = []
    # Garantir que exista um callback mesmo que a view ainda não tenha
    # registrado `show_bottom_bar` — armazenar mensagens pendentes.
    try:

        def _deferred_show_bottom_bar(
        ...
        page.app_data["login_show_bottom_bar"] = _deferred_show_bottom_bar
    except Exception:
        pass
    # Limpadores: garantir que nenhuma mensagem pendente antiga seja exibida
    try:
